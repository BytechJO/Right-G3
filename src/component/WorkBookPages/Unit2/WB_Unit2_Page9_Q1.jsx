...
const OPTIONS = ["take a taxi", "take a subway", "take a bus", "ride a bike"];
...
export default function WB_Transport_Dropdown() {
  ...
  const handleCheck = () => {
    ...
    let score = 0;
    ANSWERS.forEach((a) => {
      if (selected[a.id] === a.correct) score++;
    });
    setShowResults(true);
  ...
  const isWrong = (item) => showResults && selected[item.id] !== item.correct;
